...
const ImageUploader = () => {
  ...
  const [previewDimensions, setPreviewDimensions] = useState({ width: 800, height: 800 });
  ...
  const handleDownloadWithBackground = async () => {
    if (!processedImage || !generatedBackground) return;

    // Create canvas with AI background dimensions
    ...
    // Calculate scale factor between preview container and actual image
    const containerElement = document.querySelector('.relative.overflow-hidden');
    if (!containerElement) return;
    
    const scale = bgImage.naturalWidth / containerElement.clientWidth;
    ...
    // Draw processed image with scaled coordinates
    const originalImg = await loadImage(processedImage);
    ctx.drawImage(
      originalImg,
      scaledX,
      scaledY,
      ...
      {!processedImage ? (
        ...
        >
          ...
          <div className="space-y-4">
            ...
            <p className="text-xs text-gray-400">Supported formats: JPEG, PNG (max 5MB)</p>
          </div>
        </div>
      ) : (
        ...
        <div className="space-y-8">
          ...
          <div className="bg-white rounded-xl shadow-sm p-6 space-y-6">
            <h3 className="text-lg font-medium text-gray-700">Change Background</h3>
                    ...
                    >
                      ...
                      {isSelected && (
                        <div className="absolute inset-0">
                          {/* Resize handles with updated styling */}
                          <div className="absolute -top-1 -right-1 w-3 h-3 bg-white border-2 border-purple-500 rounded-sm cursor-ne-resize" />
                          <div className="absolute -top-1 -left-1 w-3 h-3 bg-white border-2 border-purple-500 rounded-sm cursor-nw-resize" />
                          <div className="absolute -bottom-1 -right-1 w-3 h-3 bg-white border-2 border-purple-500 rounded-sm cursor-se-resize" />
                          <div className="absolute -bottom-1 -left-1 w-3 h-3 bg-white border-2 border-purple-500 rounded-sm cursor-sw-resize" />
                        </div>
                      )}
                    </div>
                  )}
